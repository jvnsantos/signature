		...
		<Fragment>
			...
			<aside className="app-sidebar sticky" id="sidebar" onMouseOver={() => Onhover()}
				...
				<div className="main-sidebar-header">
					<Link href="/portal/dashboard" className="header-logo">
						...
						<img src={`${process.env.NODE_ENV === "production" ? basePath : ""}/assets/images/brand-logos/toggle-logo.png`} alt="logo" className="main-logo toggle-dark" />
						...
						<img src={`${process.env.NODE_ENV === "production" ? basePath : ""}/assets/images/brand-logos/toggle-logo.png`} alt="logo" className="main-logo toggle-white" />
				...
				<SimpleBar className="main-sidebar " id="sidebar-scroll">
					<nav className="main-menu-container nav nav-pills flex-column sub-open">
						...
						<ul className="main-menu" onClick={() => Sideclick()}>
							{MENUITEMS.map((levelone: any, index: number) => {
								const hasAdmin = role.includes("admin");
								const hasAllRequiredRoles = levelone?.roles?.every((item: any) => role.includes(item));

								if (!hasAdmin && levelone?.roles && !hasAllRequiredRoles) {
									return <div key={`none-${Math.random() * 100}`}></div>;
								}
								return (
									<Fragment key={`simple-bar-level-one-${index}-${Math.random() * 100}`}>
                       ...
                       ${levelone.type === 'sub' ? 'slide has-sub' : ''} ${levelone?.active ? 'open' : ''} ${levelone?.selected ? 'active' : ''}`}>
											...
											{levelone.type === "empty" ?
												...
												: ""}
											{levelone.type === "sub" ?
												<Menuloop role={role} MenuItems={levelone} level={level + 1} toggleSidemenu={toggleSidemenu} />
												: ''}
										</li>
									</Fragment>
								)
