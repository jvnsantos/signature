...
function Menuloop({ MenuItems, toggleSidemenu, level, role }: any) {
  return (
    <Fragment>
      ...
      <ul className={`slide-menu child${level}  ${MenuItems.active ? 'double-menu-active' : ''} ${MenuItems?.dirchange ? "force-left" : ""} `} style={MenuItems.active ? { display: "block" } : { display: "none" }}>
        ...
        {MenuItems.children.map((firstlevel: any, index: number) => {
          ...
          const hasAllRequiredRoles = firstlevel?.roles?.every((item: any) => role.includes(item));

          if (!hasAdmin && firstlevel?.roles && !hasAllRequiredRoles) {
            return <Fragment key={`nulable-${index}-${Math.random() * 100}`}></Fragment>;
          }

          return (<li key={`sidebar-menu-loop-${index}-${Math.random() * 100}`} className={`${firstlevel.menutitle ? 'slide__category' : ''} ${firstlevel?.type == 'empty' ? 'slide' : ''} ${firstlevel?.type == 'link' ? 'slide' : ''} ${firstlevel?.type == 'sub' ? 'slide has-sub' : ''} ${firstlevel?.active ? 'open' : ''} ${firstlevel?.selected ? 'active' : ''}`}>
